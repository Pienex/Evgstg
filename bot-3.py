...
def parse_batch_records(text):
    """Пакетная запись вида:
    «50м ноги труба тест : Аня 40; Оля 30»
    Общая часть до первого ':' применяется ко всем именам после ':', сами
    имена с временем перечисляются через ';' или с новой строки.
    Возвращает список (record, warnings) на каждого пловца, либо None, если
    текст не похож на пакетную запись.
    """
    if ":" not in text:
        return None
