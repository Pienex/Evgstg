...
def init_db():
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS standards (
            ...
            stroke TEXT NOT NULL,
            pool INTEGER NOT NULL DEFAULT 25,
            gender TEXT NOT NULL DEFAULT 'общий',
            label TEXT NOT NULL,
            time_seconds REAL NOT NULL
        )
        """
    )
    standards_cols = {row[1] for row in conn.execute("PRAGMA table_info(standards)")}
    if "gender" not in standards_cols:
        conn.execute("ALTER TABLE standards ADD COLUMN gender TEXT NOT NULL DEFAULT 'общий'")
    conn.execute(
        """
...
GENDER_LABELS = {"м": "муж", "ж": "жен", "общий": "общий"}
MALE_TOKENS = {"м", "муж", "мужской", "мужчины", "мальчики", "male", "m", "муж."}
FEMALE_TOKENS = {"ж", "жен", "женский", "женщины", "девочки", "female", "f", "жен."}


def normalize_gender(raw):
    """Возвращает 'м' / 'ж', если raw похоже на указание пола, иначе None."""
    key = raw.strip().lower()
...
def add_standard(distance, stroke, pool, gender, label, time_seconds):
    conn = sqlite3.connect(DB_PATH)
    conn.execute(
        "INSERT INTO standards (distance, stroke, pool, gender, label, time_seconds) VALUES (?, ?, ?, ?, ?, ?)",
        (distance, stroke, pool, gender, label, time_seconds),
    )
    conn.commit()
    conn.close()
...
def get_standards(distance=None, stroke=None, pool=None, gender=None):
    conn = sqlite3.connect(DB_PATH)
    query = "SELECT id, distance, stroke, pool, gender, label, time_seconds FROM standards WHERE 1=1"
    params = []
    ...
    if gender is not None:
        # показываем нормативы для выбранного пола + общие (не разделённые по полу)
        query += " AND (gender = ? OR gender = 'общий')"
        params.append(gender)
    query += " ORDER BY distance, stroke, gender, pool, time_seconds DESC"
    rows = conn.execute(query, params).fetchall()
    conn.close()
    return rows


def get_standard_distances(pool=None, gender=None):
    conn = sqlite3.connect(DB_PATH)
    query = "SELECT DISTINCT distance FROM standards WHERE 1=1"
    params = []
    if pool is not None:
        query += " AND pool = ?"
        params.append(pool)
...
def get_standard_strokes(pool=None, distance=None, gender=None):
    conn = sqlite3.connect(DB_PATH)
...
def format_standards_message(rows):
    lines = ["<b>Нормативы</b>:"]
    last_key = None
    for std_id, d, s, pool, gender, label, time_seconds in rows:
        key = (d, s, pool, gender)
        if key != last_key:
            gender_label = GENDER_LABELS.get(gender, gender)
            lines.append(f"\n<b>{d}м {s} ({gender_label}), бассейн {pool}м:</b>")
            last_key = key
...
def get_norm_status(distance, stroke, pool, time_seconds):
    ...
    if not rows:
        return None
    not_met = [r for r in rows if time_seconds > r[6]]
    if not not_met:
        best = min(rows, key=lambda r: r[6])
        return {"achieved_all": True, "top_label": best[5]}
    target = max(not_met, key=lambda r: r[6])
    return {
        "achieved_all": False,
        "label": target[5],
        "target_time": target[6],
        "gap": time_seconds - target[6],
    }
...
ADMIN_HELP = (
    ...
    "/addnorm 100 - брасс - 25 - муж - 3 юн - 1:45.00 или !добавнорм ... — добавить норматив "
    "(Дистанция - Стиль - [Бассейн -] [Пол -] Название - Время; бассейн и пол можно не указывать — "
    "25м и «общий» по умолчанию; можно сразу несколько строк одним сообщением)\n"
    "/normativy или !нормативы — все нормативы; !нормативы 100 брасс муж — по дистанции/стилю/полу\n"
...
def parse_one_addnorm(line):
    """Разбирает одну строку норматива вида
    «Дистанция - Стиль - [Бассейн -] [Пол -] Название - Время».
    ...
    if len(parts) < 4:
        return False, (
            f"«{line}» — не смог разобрать. Формат: "
            "Дистанция - Стиль - [Бассейн -] [Пол -] Название - Время"
        )
    ...
    gender = "общий"
    if rest_parts and normalize_gender(rest_parts[0]) is not None:
        gender = normalize_gender(rest_parts[0])
        rest_parts = rest_parts[1:]
    ...
    *label_parts, time_part = rest_parts
    ...
    add_standard(distance, stroke, pool, gender, label, time_seconds)
    gender_label = GENDER_LABELS.get(gender, gender)
    return True, f"✅ {distance}м {stroke} ({gender_label}), бассейн {pool}м, «{label}» — {format_time(time_seconds)}"
...
async def cmd_addnorm(message: Message):
    ...
    if not rest.strip():
        await message.answer(
            "Напиши так: /addnorm 100 - брасс - 25 - муж - 3 юн - 1:45.00\n"
            "(Дистанция - Стиль - [Бассейн -] [Пол -] Название - Время; бассейн можно "
            "не указывать — тогда 25м по умолчанию; пол можно не указывать — тогда норматив "
            "общий для обоих)\n\n"
            "Можно сразу несколько нормативов — каждый на отдельной строке:\n"
            "<code>/addnorm 50 - вольный стиль - 25 - муж - МСМК - 21.29\n"
            "50 - вольный стиль - 25 - жен - МСМК - 23.60\n"
            "50 - вольный стиль - 25 - муж - МС - 22.65</code>",
            parse_mode="HTML",
        )
        return
...
async def cmd_normativy(message: Message):
    arg = message.text.partition(" ")[2].strip()
    distance, stroke, gender = None, None, None
    if arg:
        parts = split_fields(arg)
        # пол может стоять последним словом в любом из вариантов ввода — вытащим его отдельно
        if parts and normalize_gender(parts[-1]) is not None:
    ...
    rows = get_standards(distance, stroke, gender=gender)
    if not rows:
...
async def on_nrm_pool(callback: CallbackQuery):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="👦 Мужской", callback_data=f"nrm_gender:{pool}:м")
    builder.button(text="👧 Женский", callback_data=f"nrm_gender:{pool}:ж")
    builder.button(text="Не важно", callback_data=f"nrm_gender:{pool}:_any")
    builder.adjust(2)
    await callback.message.answer(f"Бассейн {pool}м. Какой пол?", reply_markup=builder.as_markup())
    await callback.answer()


@dp.callback_query(F.data.startswith("nrm_gender:"))
async def on_nrm_gender(callback: CallbackQuery):
    _, pool_str, gender = callback.data.split(":", 2)
    pool = int(pool_str)
    builder = InlineKeyboardBuilder()
    builder.button(text="📋 Все нормативы", callback_data=f"nrm_all:{pool}:{gender}")
    builder.button(text="🎯 Определённая дистанция", callback_data=f"nrm_distpick:{pool}:{gender}")
    builder.adjust(1)
    await callback.message.answer("Что показать?", reply_markup=builder.as_markup())
    await callback.answer()
...
async def on_nrm_all(callback: CallbackQuery):
    _, pool_str, gender = callback.data.split(":", 2)
    pool = int(pool_str)
    gender_filter = None if gender == "_any" else gender
    rows = get_standards(pool=pool, gender=gender_filter)
    if not rows:
...
async def on_nrm_distpick(callback: CallbackQuery):
    ...
    pool = int(pool_str)
    gender_filter = None if gender == "_any" else gender
    distances = get_standard_distances(pool, gender_filter)
    if not distances:
        await callback.message.answer(f"Нормативов для бассейна {pool}м пока нет.")
        await callback.answer()
        return
    builder = InlineKeyboardBuilder()
    for d in distances:
        builder.button(text=f"{d}м", callback_data=f"nrm_dist:{pool}:{gender}:{d}")
    builder.adjust(3)
...
async def on_nrm_dist(callback: CallbackQuery):
    _, pool_str, gender, distance_str = callback.data.split(":", 3)
    pool, distance = int(pool_str), int(distance_str)
    gender_filter = None if gender == "_any" else gender
    strokes = get_standard_strokes(pool, distance, gender_filter)
    builder = InlineKeyboardBuilder()
    for s in strokes:
        builder.button(text=s.capitalize(), callback_data=f"nrm_final:{pool}:{gender}:{distance}:{s}")
    builder.button(text="Пропустить (все стили)", callback_data=f"nrm_final:{pool}:{gender}:{distance}:_all")
    builder.adjust(2)
...
async def on_nrm_final(callback: CallbackQuery):
    _, pool_str, gender, distance_str, stroke = callback.data.split(":", 4)
    pool, distance = int(pool_str), int(distance_str)
    gender_filter = None if gender == "_any" else gender
    stroke_filter = None if stroke == "_all" else stroke
    rows = get_standards(distance=distance, stroke=stroke_filter, pool=pool, gender=gender_filter)
    if not rows:
