...
ADMIN_HELP = (
    ...
    "<b>Команды</b>:\n"
    "/add ... — то же самое явной командой (для групп, см. /help ниже про приватность)\n"
    "/last — последние 15 записей с их id\n"
    "/delete 15 — удалить запись №15\n"
    ...
    "/edit 15 - Аня - 200 - брасс - 3:58.00 - соревы — исправить запись №15 (формат как у /add)\n"
    "/table — Excel со всеми результатами всех пловцов\n"
    "/table Имя — Excel по одному пловцу\n"
    "/addnorm 100 - брасс - 25 - 3 юн - 1:45.00 — добавить норматив (бассейн можно "
    "не указывать, по умолчанию 25м)\n"
    "/normativy — все нормативы; /normativy 100 брасс — только по дистанции/стилю\n"
    "/delnorm 5 — удалить норматив по id\n"
    "/link id Имя — привязать чужой Telegram ID к имени пловца\n"
    "/unlink id — отвязать\n"
    "/users — список привязанных пловцов\n"
    "/my — посмотреть свой личный прогресс (как у обычных пользователей)\n"
    "/addtraining 16.09-20.09 — создать разные тренировки по дням\nПосле команды бот попросит тренировку отдельно для каждого дня\n"
    "/training или !треня — тренировка на сегодня\n"
    "/prevtraining или !предтреня — предыдущая доступная тренировка\n"
    "/trainings — список тренировок (тренер)\n"
    "/deltraining 18.09 — удалить тренировку на дату (тренер)\n!результат — мои результаты\n"
)
...
USER_HELP = (
    ...
    "<code>/normativy</code> — посмотреть все нормативы, <code>/normativy 100 брасс</code> — "
    "нормативы на конкретную дистанцию и стиль.\n\n"
    "<code>/training</code> — тренировка на сегодня. Если её нет — «выходной».\n"
    "<code>/prevtraining</code> — предыдущая тренировка."
)
...
async def cmd_addnorm(message: Message):
    ...
    if len(parts) < 4:
        await message.answer("Не смог разобрать. Формат: Дистанция - Стиль - [Бассейн -] Название - Время")
        return

    distance_digits = re.sub(r"[^\d]", "", parts[0])
    if not distance_digits:
        await message.answer("Не понял дистанцию.")
        return
    distance = int(distance_digits)
    ...
    if len(rest_parts) < 2:
        await message.answer("Не хватает названия норматива и/или времени.")
        return
    *label_parts, time_part = rest_parts
    ...
    try:
        ...
        await message.answer("Не смог разобрать время норматива.")
        return

    add_standard(distance, stroke, pool, label, time_seconds)
    await message.answer(
        f"Норматив добавлен ✅ {distance}м {stroke}, бассейн {pool}м, «{label}» — {format_time(time_seconds)}"
    )
...
async def send_today_training(message: Message):
    ...
    builder.button(text="📊 Результаты", callback_data="view_my_results")
    builder.adjust(1)
